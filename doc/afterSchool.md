...
하드웨어 기반에 이야기들이 종획으로 영역의 구분없이 넘나들어서 좋았습니다.
보는 시야가 넓어서 역시 실무하시분들에 강의는 남다른듯 합니다.
질문 많은 나올 수록 좋은 강의인데 강의중에도 질문이 많이  나왔는데 듣다보니 물어 보고 싶은게 많아져서 질문이 많았습니다.
중학교때 배웠다던 V=IR부터 클럭 인터럽트 타임테이블 emf 등에 용어가 범람했고,
처음 바이오파이썬 스터디 모임때랑 비슷한 기분이였습니다. ngs, 마이크로rna ,rna시퀀싱...
실습은 다행이 라즈베리파이때 했던 꼬마 전구에 불켜기는 아니고,
비슷한 난이도에 전자기 필드 저항(emf) 측정 실습이였습니다.
실습자체는 전자기장내 변하는 저항을 값을 읽어서 출력하는 쉬운 실습이였습니다.
os를 가지고 있는 라즈베리파이는 직접 라즈베리파이에 접속하여 수행하는것과 달리
아두이노는 프로그램을 usb 포트로 바로 업로드하고 수행하는 방식이였습니다.
아두이노로 변화하는 저항 값을 받고 노트북과 아두이노 사이에 시리얼포트로
파이썬이 serial 데이터를 받아서 파이썬 내에서 값을 출력해주었습니다.
작지만 의미 있는 실습이였습니다.
장님이 눈뜻듯 다양한 가능성이 눈에 보이는 듯하였습니다.
손에 잡히지 않는 멀게 느껴졌던 것들이 손에 잡힐듯이 보여서 해보고 싶은게 많아졌네요
