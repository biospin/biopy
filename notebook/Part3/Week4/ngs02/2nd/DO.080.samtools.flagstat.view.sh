...
for SAMPLE in `ls -1d SRR*`; do
        ...
        samtools flagstat ${SAMPLE}/mapped/bwa/${SAMPLE}.bam
        echo
        echo "$ samtools view ${SAMPLE}/mapped/bwa/${SAMPLE}.bam | head -n 16"
        samtools view ${SAMPLE}/mapped/bwa/${SAMPLE}.bam | head -n 16
        echo "..."
        echo
done
